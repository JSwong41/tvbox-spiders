    ...
    def homeContent(self, filter=None):
        ...
        for site in self.sites:
            ...
            categories = [
                ...
                {
                    "type_id": f"{site['key']}_year",
                    "type_name": "年份",
                    "filters": [{"n": str(y), "v": str(y)} for y in range(2025, 2015, -1)]
                }
            ]
            classes.append({"type_id": site['key'], "type_name": site['name'], "class": categories})
        return {"class": classes}

    # 分类内容
    def categoryContent(self, tid, pg, filter, extend):
        # 区分二级分类类型
        if "_type" in tid or "_year" in tid:
            site_key = tid.split("_")[0]
            site = next((s for s in self.sites if s['key'] == site_key), None)
            if not site:
                return {"list": []}
            params = {}
            if filter:
            ...
            if not site:
                return {"list": []}
            try:
                data = json.loads(self.fetch(site['api']))
                vods = []
                for item in data.get("list", []):
                    vods.append({
